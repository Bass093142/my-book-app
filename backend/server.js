...
// 4. ✅ Orders (เพิ่มใหม่)
app.post('/api/orders', orderController.createOrder); // ลูกค้าสั่งซื้อ
app.get('/api/admin/orders', orderController.getAllOrders); // แอดมินดูออเดอร์
app.put('/api/admin/orders/:id', orderController.updateOrderStatus); // อัปเดตสถานะ
...
app.get('/api/chat/:userId', async (req, res) => {
    try {
        ...
        const [rows] = await pool.query(
            `SELECT * FROM chat_messages WHERE sender_id = ? OR receiver_id = ? ORDER BY created_at ASC`, 
            [userId, userId]
        );
        res.json(rows);
    } catch (err) { res.status(500).json({ error: err.message }); }
...
io.on('connection', (socket) => {
    ...
    socket.on('send_message', async (data) => {
        ...
        try {
            await pool.query('INSERT INTO chat_messages (sender_id, receiver_id, message) VALUES (?, ?, ?)', [data.sender_id, data.receiver_id, data.message]);
